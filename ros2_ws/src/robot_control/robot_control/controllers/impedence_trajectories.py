...
t_all = np.vstack((t_idle_to_crouch, t_crouch_to_jump + t_idle_to_crouch[-1]))
xd_d_all = np.vstack((xd_d_idle_to_crouch_traj, xd_d_crouch_to_jump_traj + xd_d_idle_to_crouch_traj[-1]))
xdd_d_all = np.vstack((xdd_d_idle_to_crouch_traj, xdd_d_crouch_to_jump_traj + xdd_d_idle_to_crouch_traj[-1]))
...
if __name__ == "__main__":
    ...
    plt.figure()
    plt.plot(t_all, xd_d_all[:, 1], label='z velocity')
    ...
    plt.figure()
    plt.plot(t_all, xdd_d_all[:, 1], label='z acceleration')
