function [u] = controller_jump(t,X,p)
params = p.params;
q = X(1:3);
dq = X(4:6);
time_stance = p.time_stance; % Stance Duration
if ~isfield(get_params(),'time_contact')
    time_contact = 0; % If Starting from the Ground
else
    time_contact = p.time_contact(1,end); % Start Time of Stance Phase
end
% Ensure that Unobservable States do not affect Controller
observability = [ 0 0 0 ;
                  0 1 0 ;
                  0 0 1 ; ] ;
q = observability * q;
dq = observability * dq;
% Control Gains in the Air (Task Space)
Kp = p.Kp_jump;
Kd = p.Kd_jump;
% Phase Variable 
% t
% p.time_contact
x = (t - time_contact)/time_stance;
p.x = [p.x x];
% x
% Ensure Phase is bounded between 0 and 1 for any Purpose
% # looks wrong
x(x<0) = 0 ;
x(x>1) = 1 ;
% Feedforward Force Trajectory (6th Order)
% Only +Y force wanted, X forces unwanted due to Gantry
F_ff = zeros(2,1);
F_ff(2) = fcn_F_traj(x,p.jump_force);
% Feedback Torque using Weak PD Controller to Stabilize System About Set Point in Task Space
x_des = [ 0 , -0.075]; % Desired Task Space Position of the Foot w.r.t Base
q_des = fcn_IK(x_des,params);
tau_fb = Kp * (q_des - q(2:3)) - Kd * dq(2:3);
% Jacobian from Hip to Foot
J14 = fcn_J14(q,params);
% Total Torque

u = 50 * (-(J14'*F_ff) + [ 0 ; tau_fb]) % '-' because it is Foot on Environment

